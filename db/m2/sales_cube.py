        ...
        return self.postgres_sql.read_qry(
            f"""
                WITH all_c_p AS (
                    ...
                    FROM  sales_cube.customers as cust, sales_cube.products pr ORDER BY cust.id
                ),
                all_c_p_amt AS (
                    SELECT
                        all_c_p.customer_id,
                    ...
                    ORDER BY all_c_p.customer_id, all_c_p.product_id ASC
                )
...
def build_options(settings):
    ...
    with open(f'db/m2/files/SalesCube_M2_leonardo.sql', 'w') as f:
